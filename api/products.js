...
const approvedBrands={
  women:new Set(['Swarovski','THOMAS SABO','Fossil','LIEBESKIND','s.Oliver','Guess','Michael Kors','Calvin Klein','Tommy Hilfiger','Elli','PANDORA','Engelsrufer','JOOP!','Skagen','Kate Spade','DKNY','JETTE','Amor','TI SENTO','ANIA HAIE']),
  men:new Set(['Fossil','Diesel','Police','Tommy Hilfiger','Emporio Armani','BOSS','Lacoste','Calvin Klein','s.Oliver','Maserati','THOMAS SABO','Guess','JOOP!','Skagen','Armani Exchange','PAUL HEWITT','Lotus Style','Sector','Philipp Plein'])
};
...
export default async function handler(req,res){
  ...
  try{
    ...
    const add=product=>{
      ...
      if(products.length>=500)return false;
      if(!product.available||!approvedBrands[gender].has(product.brand)||seen.has(key))return false;
      if(products.filter(p=>p.category===product.category).length>=170)return false;
      seen.add(key);products.push(product);return true;
    };
    for(const bucket of buckets){
      for(const product of bucket){
        add(product);
        if(products.filter(p=>p.category===product.category).length===170)break;
      }
    }
    for(const product of globalThis.SCHMUCK_PRODUCTS?.[gender]||[])add(product);
    res.setHeader('Cache-Control','s-maxage=3600, stale-while-revalidate=86400');
    return res.status(200).json({products,count:products.length,requested:500,partnerTag:PARTNER_TAG,updatedAt:new Date().toISOString()});
  }catch(error){
    const products=globalThis.SCHMUCK_PRODUCTS?.[gender]||[];
    res.setHeader('Cache-Control','s-maxage=3600, stale-while-revalidate=86400');
    return res.status(200).json({products,count:products.length,requested:500,partnerTag:PARTNER_TAG,updatedAt:globalThis.SCHMUCK_PRODUCTS?.updatedAt,source:'verified-cache'});
  }
}
