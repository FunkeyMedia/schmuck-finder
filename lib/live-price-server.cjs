function createPriceService({ allowed, gatewayUrl = process.env.AMAZON_CATALOG_GATEWAY_URL, secret = process.env.AMAZON_CATALOG_GATEWAY_SECRET, fetchImpl = fetch, now = Date.now }) {
