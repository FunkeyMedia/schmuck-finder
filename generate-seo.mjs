...
function shell({title, description, route, content, schema=[], image='/assets/references/nasteho-elin.webp', bodyClass='', scripts=''}) {
  const canonical = `${origin}/${route ? `${route}/` : ''}`;
  return `<!doctype html><html lang="de"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${canonical}"><meta property="og:type" content="website"><meta property="og:site_name" content="SCHMUCK FINDER"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="${origin}${image}"><meta property="og:image:alt" content="Redaktionelles Motiv von SCHMUCK FINDER"><meta name="twitter:card" content="summary_large_image"><meta name="theme-color" content="#3d2330"><link rel="icon" href="/assets/brand/mark.svg" type="image/svg+xml"><link rel="stylesheet" href="/seo.css">${schema.map(structured).join('')}</head><body class="${bodyClass}"><a class="skip" href="#content">Zum Inhalt</a><div class="announcement">FINDE DAS STÜCK, DAS SICH NACH DIR ANFÜHLT.</div><header><a class="brand" href="/"><img src="/assets/brand/logo-light.svg" alt="SCHMUCK FINDER"></a><nav aria-label="Hauptnavigation"><a href="/frauen/">Women</a><a href="/maenner/">Men</a><a href="/finder/">Finder</a><a href="/marken/">Marken</a><a href="/ratgeber/">Journal</a></nav></header><main id="content">${content}</main>${footer()}${scripts}<script src="/tracking.js"></script></body></html>`;
}
function footer(){ return `<footer><div><img src="/assets/brand/logo-light.svg" alt="SCHMUCK FINDER"><p>Finde das Stück, das sich nach dir anfühlt.</p></div><nav aria-label="Fußnavigation"><a href="/finder/">Schmuck-Finder</a><a href="/frauen/">Schmuck für Frauen</a><a href="/maenner/">Schmuck für Männer</a><a href="/themen/">Themen</a><a href="/ratgeber/">Journal</a><a href="/methodik/">So wählen wir aus</a><a href="/redaktion/">Redaktion</a><a href="/#impressum">Impressum</a><a href="/#datenschutz">Datenschutz</a></nav><p class="legal">Als Amazon-Partner verdienen wir an qualifizierten Verkäufen. Preise und Verfügbarkeit können sich ändern; maßgeblich sind die Angaben bei Amazon.de.</p></footer>`; }
...
function productCards(products){ return `<div class="product-grid">${products.slice(0,12).map(p=>`<article class="product-card"><a href="${esc(p.url)}" rel="nofollow sponsored noopener" target="_blank" data-product-click data-asin="${esc(p.asin)}" data-brand="${esc(p.brand)}"><img src="${esc(p.image)}" alt="${esc(p.brand)} ${esc(p.title)}" loading="lazy" width="500" height="500"><span>${esc(p.brand)}</span><h2>${esc(p.title)}</h2><strong>${esc(p.price || 'Preis bei Amazon prüfen')}</strong><b>Bei Amazon ansehen →</b></a></article>`).join('')}</div>`; }
...
const finderItems=[{name:'Startseite',url:'/'},{name:'Schmuck-Finder',url:'/finder/'}];
const finderIntro=`${crumbs(finderItems)}<section class="hero compact"><p class="eyebrow">In vier Schritten zur passenden Auswahl</p><h1>Schmuck finden, der zu dir passt</h1><p>Grenze 1.000 Markenstücke nach Zielgruppe, Schmuckart, Material, Marke und Budget ein. SCHMUCK FINDER verkauft nicht selbst; Preis und Verfügbarkeit bestätigst du beim verlinkten Händler.</p></section><section class="finder-tool" aria-labelledby="finder-heading"><div class="finder-toolbar"><label>Zielgruppe<select id="finder-gender"><option value="women">Women</option><option value="men">Men</option></select></label><label>Schmuckart<select id="finder-category"><option value="">Alle Schmuckarten</option>${categories.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select></label><label>Material<select id="finder-material"><option value="">Alle Materialien</option><option value="gold">Goldfarben</option><option value="silber">Silberfarben</option><option value="edelstahl">Edelstahl</option></select></label><label>Budget<select id="finder-budget"><option value="">Jedes Budget</option><option value="50">Unter 50 Euro</option><option value="100">50 bis 100 Euro</option><option value="101">Ab 100 Euro</option></select></label><label>Marke oder Modell<input id="finder-search" type="search" placeholder="z. B. Swarovski Kette"></label><button id="finder-reset" type="button">Auswahl zurücksetzen</button></div><div class="finder-result-head"><h2 id="finder-heading">Aktuelle Auswahl</h2><p id="finder-count">${catalog.women.length} Treffer für Women</p></div><div id="finder-results">${productCards(catalog.women)}</div><button id="finder-more" class="button" type="button">Mehr Schmuck anzeigen</button><p class="editorial-note">Produktdaten: Amazon Creators API mit geprüftem Fallback-Katalog, Datenstand ${new Intl.DateTimeFormat('de-DE').format(new Date(catalog.updatedAt))}. Als Amazon-Partner verdienen wir an qualifizierten Verkäufen. Maßgeblich sind Preis und Verfügbarkeit auf Amazon.de.</p></section><section class="answer"><h2>So triffst du eine belastbare Auswahl</h2><p>Beginne mit Schmuckart und Material. Nutze anschließend Marke und Budget, um die Auswahl zu verkleinern. Vergleiche zwei oder drei Stücke nach Maßen, Verschluss, Materialangaben und Alltagstauglichkeit. Produktfotos allein zeigen Größe und Gewicht nicht zuverlässig.</p><div class="related"><a href="/ratgeber/materialien/">Materialien vergleichen →</a><a href="/ratgeber/ringgroesse/">Ringgröße messen →</a><a href="/ratgeber/geschenke/">Schmuck schenken →</a></div></section>`;
write('finder',shell({title:'Schmuck-Finder: Marken, Material und Budget filtern',description:'1.000 Markenstücke für Frauen und Männer nach Schmuckart, Material, Marke und Budget filtern – mit Ratgebern für eine sichere Auswahl.',route:'finder',content:finderIntro,schema:[websiteSchema,breadcrumbSchema(finderItems)],scripts:'<script src="/products-data.js"></script><script src="/finder-page.js"></script>'}));
...
const brandItems=[{name:'Startseite',url:'/'},{name:'Marken',url:'/marken/'}];
const brandLinks=brands.map(brand=>{
  const count=[...catalog.women,...catalog.men].filter(p=>p.brand===brand).length;
  return count>=8?`<a class="choice" href="/marken/${slug(brand)}/"><strong>${esc(brand)}</strong><span>${count} Modelle entdecken →</span></a>`:'';
}).join('');
write('marken',shell({title:'Schmuckmarken vergleichen | SCHMUCK FINDER',description:'Bekannte Schmuckmarken für Frauen und Männer vergleichen: Modelle, Schmuckarten und aktuelle Händlerangebote übersichtlich entdecken.',route:'marken',content:`${crumbs(brandItems)}<section class="hero compact"><p class="eyebrow">Markenwelt</p><h1>Schmuckmarken vergleichen</h1><p>Entdecke Marken aus dem geprüften Katalog und vergleiche ihre verfügbaren Ketten, Ringe, Armbänder, Ohrringe und Anhänger. SCHMUCK FINDER verkauft nicht selbst.</p><a class="button" href="/finder/">Alle Marken im Finder filtern</a></section><section><h2>Marken im Katalog</h2><div class="choices">${brandLinks}</div></section>`,schema:[breadcrumbSchema(brandItems)]}));
...
const topicItems=[{name:'Startseite',url:'/'},{name:'Themen',url:'/themen/'}];
const topicGroups=[
  ['materialien','Materialien und Verträglichkeit','Gold, Silber, Edelstahl, Beschichtungen und empfindliche Haut verständlich einordnen.',['materialien','gold-silber','vergoldung','allergien','nachhaltigkeit']],
  ['groessen','Größen und Passform','Ringgröße, Kettenlänge und Verschlüsse vor dem Kauf verlässlich prüfen.',['ringgroesse','kettenlaenge','verschluss']],
  ['geschenke','Schmuck als Geschenk','Stil, Bedeutung, Größe und Alltagstauglichkeit für Geschenke zusammenbringen.',['geschenke','personalisierter-schmuck','hochzeit']],
  ['stil','Stil und Kombination','Schmuck kombinieren, Layering planen und eine persönliche Stilrichtung entwickeln.',['layering','ring-stacking','ringe-kombinieren','business','statement-schmuck','herrenschmuck']]
];
const topicLinks=topicGroups.map(([route,title,desc])=>`<a class="choice" href="/themen/${route}/"><strong>${title}</strong><span>${desc} →</span></a>`).join('');
write('themen',shell({title:'Schmuck-Themen: Materialien, Größen, Geschenke und Stil',description:'Alle wichtigen Schmuckthemen geordnet nach Materialien, Größen, Geschenken und Stil – mit Ratgebern und passenden Produktauswahlen.',route:'themen',content:`${crumbs(topicItems)}<section class="hero compact"><p class="eyebrow">Orientierung nach Aufgabe</p><h1>Schmuckwissen nach Themen</h1><p>Beginne mit deiner konkreten Frage statt mit einer endlosen Produktliste. Vier Themenwege führen zu den passenden Ratgebern und Auswahlen.</p></section><section><h2>Was möchtest du entscheiden?</h2><div class="choices">${topicLinks}</div></section>`,schema:[breadcrumbSchema(topicItems)]}));
for(const [topicRoute,title,desc,slugs] of topicGroups){
  const route=`themen/${topicRoute}`;
  const items=[...topicItems,{name:title,url:`/${route}/`}];
  const selected=articles.filter(a=>slugs.includes(a[0]));
  const links=selected.map(a=>`<a class="choice" href="/ratgeber/${a[0]}/"><strong>${esc(a[1])}</strong><span>${esc(a[4])} →</span></a>`).join('');
  write(route,shell({title:`${title} | SCHMUCK FINDER`,description:desc,route,content:`${crumbs(items)}<section class="hero compact"><p class="eyebrow">Themenweg</p><h1>${title}</h1><p>${desc}</p><a class="button" href="/finder/">Passende Produkte filtern</a></section><section><h2>Passende Ratgeber</h2><div class="choices">${links}</div></section>`,schema:[breadcrumbSchema(items)]}));
}

const methodItems=[{name:'Startseite',url:'/'},{name:'So wählen wir aus',url:'/methodik/'}];
write('methodik',shell({title:'So wählt SCHMUCK FINDER Produkte und Themen aus',description:'Transparente Methodik: Datenquellen, Auswahlkriterien, Aktualisierung und Grenzen der Produktempfehlungen von SCHMUCK FINDER.',route:'methodik',content:`${crumbs(methodItems)}<article class="article"><header><p class="eyebrow">Transparenz</p><h1>So wählen wir Produkte und Themen aus</h1><p class="lead">SCHMUCK FINDER verbindet strukturierte Produktdaten mit redaktionellen Entscheidungshilfen. Wir führen keine eigenen Material-, Labor- oder Tragetests durch.</p></header><section><h2>Produktdaten</h2><p>Bilder, Titel, Preise, Links und Verfügbarkeitsangaben stammen aus der Amazon Creators API. Ein am 12. September 2026 geprüfter Fallback-Katalog hält den Finder bei zeitweisen API-Problemen nutzbar. Verbindlich sind immer die Angaben beim Händler zum Kaufzeitpunkt.</p><h2>Auswahlkriterien</h2><p>Produkte werden nach Zielgruppe, Schmuckart, Marke, Materialbegriffen und Preisbereich eingeordnet. Redaktionelle Seiten wählen daraus passende Beispiele. Eine Aufnahme ist keine Testnote und keine Aussage über individuelle Passform, Materialverträglichkeit oder Wert.</p><h2>Reihenfolge</h2><p>Der Finder ordnet standardmäßig nach der vorhandenen Katalogreihenfolge und bietet zusätzlich Preis- und Markenfilter. Bezahlte Einzelplatzierungen innerhalb der redaktionellen Auswahl sind nicht vorgesehen.</p><h2>Aktualisierung und Fehler</h2><p>Produktdaten werden über die Schnittstelle aktualisiert und gegen den Fallback ergänzt. Fehlerhinweise nimmt die Redaktion unter <a href="mailto:info@schmuck-finder.de">info@schmuck-finder.de</a> entgegen.</p><h2>Affiliate-Finanzierung</h2><p>Als Amazon-Partner verdient SCHMUCK FINDER an qualifizierten Verkäufen. Für Nutzer ändert sich der Preis dadurch nicht. Die Affiliate-Finanzierung ersetzt keine unabhängige Prüfung der Produktangaben.</p></section></article>`,schema:[breadcrumbSchema(methodItems)]}));

const editorialItems=[{name:'Startseite',url:'/'},{name:'Redaktion',url:'/redaktion/'}];
write('redaktion',shell({title:'SCHMUCK FINDER Redaktion und Verantwortung',description:'Wer hinter SCHMUCK FINDER steht, wie Inhalte entstehen und wie du die Redaktion bei Fragen oder Fehlerhinweisen erreichst.',route:'redaktion',content:`${crumbs(editorialItems)}<article class="article"><header><p class="eyebrow">Verantwortung</p><h1>Die SCHMUCK FINDER Redaktion</h1><p class="lead">SCHMUCK FINDER ist ein redaktioneller Online-Finder von marktSTARK®, Inhaber Pascal Weyers. Die Redaktion strukturiert Produktdaten und erstellt Entscheidungshilfen zu Material, Größe, Pflege, Stil und Geschenken.</p></header><section><h2>Redaktionelle Verantwortung</h2><p>Verantwortlich gemäß den Angaben im Impressum ist Pascal Weyers. Der Unternehmensstandort befindet sich in Obertshausen. SCHMUCK FINDER betreibt keinen Onlineshop und keine im Projekt ausgewiesenen Filialen.</p><h2>Arbeitsweise</h2><p>Ratgeber trennen Hersteller- und Händlerangaben von redaktioneller Einordnung. Quellen werden dort verlinkt, wo sie eine fachliche Aussage stützen. Grenzen und Unsicherheiten werden sichtbar benannt.</p><h2>Kontakt und Korrekturen</h2><p>Fragen, Ergänzungen und Fehlerhinweise können an <a href="mailto:info@schmuck-finder.de">info@schmuck-finder.de</a> gesendet werden.</p><p><a class="button" href="/methodik/">Auswahlmethodik lesen</a></p></section></article>`,schema:[breadcrumbSchema(editorialItems),organizationSchema]}));
