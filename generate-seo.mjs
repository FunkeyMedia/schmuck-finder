...
const journalCards=articles.map((a,i)=>`<article class="journal-card"><a href="/ratgeber/${a[0]}/"><img src="${editorial[i%editorial.length]}" alt="Redaktionelles Schmuckmotiv" loading="lazy" width="640" height="800"><span>${esc(a[2])} · ${esc(a[3])}</span><h2>${esc(a[1])}</h2><p>${esc(a[4])}</p></a></article>`).join('');
...
for (let i=0;i<articles.length;i++) {
  const a=articles[i], d=details[a[0]], route=`ratgeber/${a[0]}`, image=editorial[i%editorial.length];
